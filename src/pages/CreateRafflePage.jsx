...
export default function CreateRafflePage() {
  ...
  const [raffleCategory, setRaffleCategory] = useState('crypto');
  const [prizeType, setPrizeType] = useState('token');
  ...
  // Determine what we're showing based on category + prize type
  const isAvalanche = raffleCategory === 'avalanche';
  const isItemPrize = prizeType === 'item';
  const isTokenPrize = prizeType === 'token';
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px 80px' }}>
      ...
      <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 32 }}>
        {isAdmin
          ...
          : 'List crypto tokens or NFTs for raffle and let the community compete for a chance to win.'}
      </p>

      {/* Category Toggle (Admin sees both, users see only Community) */}
      <div style={{
        ...
        <button
          onClick={() => { setRaffleCategory('crypto'); setPrizeType('token'); }}
          style={{
            padding: '10px 24px', borderRadius: 8, border: 'none',
            background: raffleCategory === 'crypto' ? 'var(--avax-red)' : 'transparent',
            color: raffleCategory === 'crypto' ? 'white' : 'var(--text-secondary)',
        ...
        {isAdmin && (
          <button
            onClick={() => { setRaffleCategory('avalanche'); setPrizeType('item'); }}
            style={{
              padding: '10px 24px', borderRadius: 8, border: 'none',
              background: raffleCategory === 'avalanche' ? 'var(--avax-red)' : 'transparent',
              color: raffleCategory === 'avalanche' ? 'white' : 'var(--text-secondary)',
              fontSize: 14, fontWeight: 600, cursor: 'pointer',
              transition: 'all 0.2s',
              display: 'flex', alignItems: 'center', gap: 8,
            }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <textarea
                placeholder={isAvalanche
                  ? 'Describe the prize and raffle details...'
                  : 'Tell people about this raffle...'}
                rows={3}
            ...
            {/* Raffle Type selector — Token or NFT */}
            <div>
              <label style={labelStyle}>Raffle type <span style={{ color: 'var(--avax-red)' }}>*</span></label>
              <div style={{ display: 'flex', gap: 0, background: 'var(--surface)', borderRadius: 10, padding: 4 }}>
                {isAvalanche && (
                  <button
                    onClick={() => setPrizeType('item')}
                    style={{
                      flex: 1, padding: '10px 0', borderRadius: 8, border: 'none',
                      background: isItemPrize ? 'var(--avax-red)' : 'transparent',
                      color: isItemPrize ? 'white' : 'var(--text-secondary)',
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: '10px 0', borderRadius: 8, border: 'none',
                    background: isTokenPrize ? 'var(--avax-red)' : 'transparent',
                    color: isTokenPrize ? 'white' : 'var(--text-secondary)',
                    fontSize: 13, fontWeight: 600, cursor: 'pointer',
                    transition: 'all 0.2s',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                  }}
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: '10px 0', borderRadius: 8, border: 'none',
                    background: isNftPrize ? 'var(--avax-red)' : 'transparent',
                    color: isNftPrize ? 'white' : 'var(--text-secondary)',
                    fontSize: 13, fontWeight: 600, cursor: 'pointer',
                    transition: 'all 0.2s',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                  }}
            ...
            {isTokenPrize ? (
              ...
              <>
                {/* Item or NFT — Image upload */}
                <div>
                  <label style={labelStyle}>{isItemPrize ? 'Item image' : 'NFT image'} <span style={{ color: 'var(--avax-red)' }}>*</span></label>
                  <div
                  ...
                  >
                    {imagePreview ? (
                      ...
                      <div style={{ color: 'var(--text-muted)' }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 500 }}>Click to upload {isItemPrize ? 'item' : 'NFT'} image</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{
            ...
            <div style={{
              background: isTokenPrize && !isAvalanche
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              {isAvalanche ? 'How Avalanche raffles work' : 'How community raffles work'}
            </h3>
            {(isItemPrize ? [
              { step: '1', text: 'Fill out the details of the raffle' },
              { step: '2', text: 'Upload an image and set raffle details' },
              { step: '3', text: 'Once sold out or time expires, Chainlink VRF draws the winner' },
              { step: '4', text: 'Winner is selected and can elect to receive item or a cash alternative' },
              { step: '5', text: 'Item is shipped or cash alternative is sent' },
            ] : isTokenPrize ? [
              ...
              { step: '1', text: 'Upload your NFT image and set raffle details' },
              { step: '2', text: 'Approve and transfer your NFT into escrow' },
              { step: '3', text: 'NFT is locked in the smart contract until the draw' },
              { step: '4', text: 'Once sold out or time expires, Chainlink VRF draws the winner' },
              { step: '5', text: 'Winner receives the NFT, you receive the AVAX from ticket sales' },
            ]).map(item => (
          ...
          <div style={{
            ...
            {[
              ...
              { label: 'Winner receives', value: isItemPrize ? 'Item or cash alternative' : isNftPrize ? 'NFT (100%)' : isAvalanche ? 'Prize (100%)' : 'Tokens (100%)', color: 'var(--text-primary)' },
            ].map(row => (
